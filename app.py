...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    # Contatore grezzo da OpenInsider prima di qualsiasi filtro
    raw_count   = len(trades)
    raw_tickers = len({t["ticker"] for t in trades})
    st.markdown(
        ...
        f'&#x1F4E1; <b style="color:#8b949e">OpenInsider raw</b> &rarr; '
        f'<b style="color:#10b981">{raw_count}</b> acquisti · '
        f'<b style="color:#10b981">{raw_tickers}</b> titoli '
        f'(soglia ${vl}K · ultimi 30gg) — i filtri sotto riducono questa lista'
        f'</div>',
        unsafe_allow_html=True)
