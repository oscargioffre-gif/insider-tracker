"""
OS OpenInSider  v11
• Fonte primaria: http://openinsider.com (HTTP porta 80, Chrome UA, retry x3)
• Fonte secondaria: mostra banner se OI non raggiungibile
• Latenza: ~30-60 min (OpenInsider indicizza EDGAR ogni ora)
• UI: invariata
"""
...
SECTOR_KEYWORDS = {
    "Semiconductors": [
        "semiconductor","semiconductors","semi ","microchip","integrated circuit",
        "nvidia","intel","amd","qualcomm","broadcom","texas instrument","applied material",
        "lam research","kla ","asml","marvell","skyworks","qorvo","on semi","onsemi",
        "wolfspeed","lattice","maxim","analog device","microchip tech","silicon",
        "monolithic","photronics","axcelis","ultratech","entegris","cabot micro",
        "kulicke","amkor","cohu","ichor","cree","allegro","power integr","vishay",
        "diodes inc","semtech","indie semi","indie","ceva","alphawave","transcode",
        "magnachip","tower semi","pdf solution","pdf sol","aehr","atomica",
    ],
    "Healthcare": [
        "health","pharma","medical","biotech","bio ","therapeut","clinical",
        "hospital","drug","life science","genomic","oncolog","bioscience",
        "biopharma","lifescience","medtech","biomed","diagnostic","surgical",
        "cardio","neuro","immuno","vaccine","antibod","peptide","cell therapy",
        "gene therapy","opthalm","dental","radiol","patholog","laborator",
        "quest diag","labcorp","medtronic","abbvie","pfizer","merck","lilly",
        "amgen","gilead","regeneron","vertex","moderna","biontech","novavax",
        "biogen","alexion","shire","celgene","myovant","natera","guardant",
        "insulet","dexcom","intuitive","hologic","becton","baxter","zimmer",
        "edwards life","abiomed","resmed","natus","nuo therap","vivani",
    ],
    "Technology": [
        "tech","software","cloud","cyber","digital","platform","saas","data",
        "artificial intel","machine learn","network","internet","computing",
        "information","solutions","systems inc","it inc","it corp","software inc",
        ...
        "toast","braze","appfolio","instructure","duolingo","coursera",
    ],
    "Industrials": [
        ...
        "caterpillar","deere","honeywell","ge ","general electric","3m ","mmm",
        "lockheed","raytheon","northrop","boeing","airbus","parker hannifin",
        "emerson","rockwell","eaton","dover","illinois tool","ite ","itt ",
        "ametek","watts water","roper","xylem","watts","kennametal","timken",
        "graco","nordson","lincoln electric","flowserve","curtiss","hexcel",
        "spirit aero","heico","transdigm","moog","woodward","kaman","crane",
        "watts water","watsco","aaon","comfort systems","insteel","mueller",
    ],
}
...
def fetch_trades(vl, vh):
    """Fetch da OpenInsider. Cache 5 min. Fonte: openinsider.com"""
    ...
    trades = [t for t in trades if t["value"] >= vl_val and t["value"] <= vh_val]
    return trades, ""
...
def render_card(t):
    ...
    sector_badge_html = (f'<span class="badge" style="color:{sc};background:{sbg};border:1px solid {sbd};font-size:.72rem;font-weight:700;padding:2px 9px;border-radius:20px;">{sector}</span>'
                         if sector != "—" else "")
    st.markdown(f"""
<div class="card">
  <div class="card-top">
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    # Filtro settori (classificazione per nome azienda — no API esterne)
    if sel_sectors:
...
<div class="stats-bar">
  ...
  <div class="stat-ts">{now_s} IT &middot; OpenInsider &middot; C-Level + 10% Owner + Director &middot; Form 4</div>
</div>""", unsafe_allow_html=True)
