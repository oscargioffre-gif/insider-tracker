...
OS OpenInSider  v10
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Fonte dati: SEC EDGAR diretto
  1. EFTS keyword search  →  pre-filtra Form 4 C-Level/10%
  2. Form 4 XML fetch parallelo (5 workers)  →  parsing nativo
Latenza: ~5-15 min vs ~30-60 min di OpenInsider
Cache:   120 secondi
UI:      identica alla v9
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
EDGAR_HDR  = {
    "User-Agent":      "OS-OpenInSider-Tracker os-insider-tracker@proton.me",
    "Accept-Encoding": "gzip, deflate",
    "Accept":          "application/json, text/xml, */*",
}
# Keyword query: matches Form 4 filings that contain C-Level titles or 10% owner
EDGAR_QUERY = (
    '"Chief Executive" OR "Chief Financial" OR "Chief Operating" OR '
    '"Chief Technology" OR "Chief Medical" OR "Chief Scientific" OR '
    '"Chief Legal" OR "Chairman" OR "President" OR "General Counsel" OR '
    '"10% Owner" OR "10 percent"'
)


# ─── OPENINSIDER CONSTANTS (fallback) ─────────────────────────────────────────
OI_HEADERS = {
    "User-Agent": (
...
EXCLUDE_KW = ["director","beneficial owner"]
SECTOR_THEME = {
...
# ─── EDGAR: XML FETCH ─────────────────────────────────────────────────────────
def _edgar_fetch_xml(accession_raw):
    """
    Fetch Form 4 XML from EDGAR.
    Strategy: try form4.xml first (most common), then parse filing index.
    Returns XML text string or None.
    """
    try:
        acc_nodash = accession_raw.replace("-", "")
        cik        = str(int(accession_raw.split("-")[0]))  # strip leading zeros

        # Attempt 1: standard filename
        url1 = f"{EDGAR_BASE}/{cik}/{acc_nodash}/form4.xml"
        r1   = requests.get(url1, headers=EDGAR_HDR, timeout=8)
        if r1.status_code == 200:
            return r1.text

        # Attempt 2: filing index to discover actual filename
        idx_url = f"{EDGAR_BASE}/{cik}/{acc_nodash}/{accession_raw}-index.json"
        ri      = requests.get(idx_url, headers=EDGAR_HDR, timeout=8)
        if ri.status_code != 200:
            return None
        for doc in ri.json().get("documents", []):
            if doc.get("type") == "4" and doc.get("document","").endswith(".xml"):
                url2 = f"{EDGAR_BASE}/{cik}/{acc_nodash}/{doc['document']}"
                r2   = requests.get(url2, headers=EDGAR_HDR, timeout=8)
...
# ─── EDGAR: XML PARSER ────────────────────────────────────────────────────────
def _parse_form4_xml(xml_text, filing_date, filing_time):
    """
    Parse a Form 4 XML and return list of purchase transaction dicts.
    Filters: transactionCode == P, acquiredDisposed == A, C-Level or 10% owner.
    """
    try:
    ...
    for owner in root.findall("reportingOwner"):
        ...
        # Include 10% owners always; officers only if C-Level
        if not is_officer and not is_10pct:
            continue
        if is_10pct:
            if not title:
                title = "10% Owner"
        ...
        if is_10pct and not is_officer:
            role_abbr, role_full = "10%", "10% Owner"
        ...
        for txn in nd_table.findall("nonDerivativeTransaction"):
            ...
            # Must be an acquisition (A)
            acq = amounts.findtext("transactionAcquiredDisposedCode/value","A")
            if acq == "D":
                continue
...
# ─── EDGAR: MAIN FETCH (cached 2 min) ────────────────────────────────────────
@st.cache_data(ttl=120, show_spinner=False)
def _fetch_edgar(vl, vh):
    """
    Fetch Form 4 da SEC EDGAR. Restituisce lista trade o [] se fallisce.
    Separato da _fetch_oi per evitare problemi con @st.cache_data.
    """
    ...
    def process_hit(hit):
        try:
            ...
            if "T" in fd:
                filing_date = fd[:10]
                filing_time = fd[11:19]
            else:
                filing_date = fd[:10] if fd else ""
                filing_time = ""
            xml_text = _edgar_fetch_xml(acc)
            if not xml_text:
                return []
            return _parse_form4_xml(xml_text, filing_date, filing_time)
        except Exception:
            return []

    all_trades = []
    with ThreadPoolExecutor(max_workers=5) as executor:
        futs = {executor.submit(process_hit, h): h for h in hits_all}
        for fut in as_completed(futs):
            try:
                all_trades.extend(fut.result(timeout=25))
            except Exception:
                pass

    vl_val = vl * 1000
    vh_val = vh * 1000 if vh > 0 else float("inf")
    all_trades = [t for t in all_trades
                  if t["value"] >= vl_val and t["value"] <= vh_val]
...
def _fetch_oi(vl, vh):
    """Fetch da OpenInsider. Cache 5 min. Separato da EDGAR."""
    trades, err = _fetch_from_oi(vl, vh)
    return trades


def fetch_trades(vl, vh):
    """
    Orchestratore: tenta EDGAR (15-min cache), fallback su OpenInsider (5-min cache).
    NON decorato con @st.cache_data — lo gestiscono le funzioni interne.
    Restituisce: (trades, source_label, warning_message)
    """
    edgar_trades = _fetch_edgar(vl, vh)
    if edgar_trades:
        return edgar_trades, "SEC EDGAR", ""
    # Fallback
    oi_trades = _fetch_oi(vl, vh)
    if oi_trades:
        return oi_trades, "OpenInsider", "⚠️ EDGAR non disponibile — dati da OpenInsider (latenza 30-60 min)"
    return [], "—", "Nessuna fonte raggiungibile"
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level Purchases &middot; SEC Form 4</div>
    ...
    # Invalidate se vl/vh cambiati
    last_vl = st.session_state.get("last_vl")
    last_vh = st.session_state.get("last_vh")
    if last_vl is not None and (last_vl != vl or last_vh != vh):
        st.session_state.pop("show_results", None)
        st.session_state.pop("show_count", None)
    st.session_state["last_vl"] = vl
    st.session_state["last_vh"] = vh
    ...
    trades, data_source, err = fetch_trades(vl, vh)
    prog.progress(60, text="&#x1F4CA; Elaborazione Form 4...")
    ...
    # Filtro settori con match flessibile
    if sel_sectors:
        def sector_match(ts, chosen):
            ts_l = ts.lower()
            return any(c.lower() in ts_l or ts_l in c.lower() for c in chosen)
        trades = [t for t in trades
                  if sector_match(enriched_map[t["ticker"]].get("sector","—"), sel_sectors)]
    if not trades:
        st.markdown(
            '<div class="empty-state"><div class="ei">&#x1F3ED;</div>'
...
<div class="stats-bar">
  ...
  <div class="stat-ts">{now_s} IT &middot; {data_source} &middot; C-Level + 10% Owner &middot; Form 4</div>
</div>""", unsafe_allow_html=True)
