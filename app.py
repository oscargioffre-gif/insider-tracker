"""
OS OpenInSider  v11
• Fonte primaria: SEC EDGAR incrementale (5-15 min latenza)
• Fonte secondaria: OpenInsider principale + SIC-filtered (30-60 min)
• Merge automatico: dati EDGAR + OI unificati, nessun dato perso
• UI: invariata
"""
...
SECTOR_COLORS["Non classificati"] = ("#94a3b8","rgba(148,163,184,.12)","rgba(148,163,184,.3)")
...
SECTOR_SIC_RANGES = {
    "Biotech": [
        ...
        (2860, 2869),   # industrial chemicals & synthetics
        (5047, 5047),   # medical & hospital equipment wholesale
        (5122, 5122),   # drugs, drug proprietaries
        (8000, 8099),   # health services
        (8710, 8734),   # engineering, research, testing services
    ],
    "Semiconductors": [
        (3670, 3679),   # electronic components & accessories
        (3825, 3827),   # instruments for measuring
    ],
    "Technology": [
        (3570, 3579),   # computer & office equipment
        (3669, 3669),   # communications equipment
    ...
    "Industrials": [
        ...
        (3710, 3799),   # transportation equipment
        (3720, 3729),   # aircraft & parts
        (4400, 4599),   # water, air transport
        (3812, 3812),   # defense systems
...
def build_url_sic(vl: int, sicl: int, sich: int, days: int = 10) -> str:
    """URL OpenInsider con filtro SIC range specifico."""
    return (
        f"http://openinsider.com/screener?s=&o=&pl=&ph=&ll=&lh="
        f"&fd={days}&fdr=&td=0&tdr=&fdlyl=&fdlyh=&daysago="
        f"&xp=1&xs=0&xa=0&xd=0&xg=0&xf=0&xm=0&xx=0&xo=0"
...
def fetch_sector_trades(vl: int, sector: str) -> list:
    """
    Fetch settoriale usando i SIC code ufficiali SEC su OpenInsider.
    Restituisce trade già taggati con sector=sector.
    Completamente indipendente dal keyword matching.
    """
    ranges = SECTOR_SIC_RANGES.get(sector, [])
    if not ranges:
        return []
    ...
    for sicl, sich in ranges:
        try:
            r = oi_safe_fetch(build_url_sic(vl, sicl, sich))
            if r is None:
                continue
            soup = BeautifulSoup(r.text, "lxml")
            rows = _parse_table(soup, cutoff)
            for t in rows:
                key = (t["ticker"], t["insider"], t["trade_date"], round(t["value"], -2))
                if key not in seen:
                    seen.add(key)
                    t["sector"] = sector  # tag autoritativo da SIC
                    all_trades.append(t)
        except:
            continue
...
def fetch_trades(vl, vh):
    """
    Tri-source system con fallback:
    1. EDGAR incrementale (RSS + XML solo nuovi) → latenza 5-15 min
    2. OpenInsider principale → latenza 30-60 min
    3. OpenInsider SIC-filtered → completamento settori
    Merge + dedup. Fonte mostrata in UI.
    """
    vl_val = vl * 1000
    cutoff = datetime.now() - timedelta(days=31)
    merged: dict = {}
    ...
    # ── FONTE 1: EDGAR incrementale ──────────────────────────────────────────
    edgar_trades, edgar_note = fetch_from_edgar(vl)
    edgar_ok = edgar_note in ("EDGAR", "edgar_ok_no_new")

    if edgar_note == "EDGAR" and edgar_trades:
        source_label = "EDGAR"
        for t in edgar_trades:
            key = (t["ticker"], t["insider"], t["trade_date"], round(t["value"], -2))
            merged[key] = t

    # ── FONTE 2: OpenInsider principale (sempre eseguito per completezza) ─────
    oi_main = []
    r = oi_safe_fetch(build_url(vl, vh))
    if r is not None:
        soup = BeautifulSoup(r.text, "lxml")
        oi_main = _parse_table(soup, cutoff)
        oi_main = [t for t in oi_main if t["value"] >= vl_val]
        if oi_main and source_label == "EDGAR":
    ...
    for t in oi_main:
        ...
        if key not in merged:
            merged[key] = t
        elif t.get("sector","—") != "—" and merged[key].get("sector","—") == "—":
            merged[key]["sector"] = t["sector"]

    # ── FONTE 3: OpenInsider SIC-filtered (settori esatti) ────────────────────
    def fetch_one_range(sector, sicl, sich):
        try:
            r2 = oi_safe_fetch(build_url_sic(vl, sicl, sich))
            if r2 is None:
                return []
            soup2 = BeautifulSoup(r2.text, "lxml")
            rows = _parse_table(soup2, cutoff)
            result = []
            for t in rows:
                if t["value"] >= vl_val:
                    t["sector"] = sector
    ...
    with ThreadPoolExecutor(max_workers=6) as ex:
        ...
        for sector, ranges in SECTOR_SIC_RANGES.items():
            for sicl, sich in ranges:
                futs.append(ex.submit(fetch_one_range, sector, sicl, sich))
        for fut in as_completed(futs, timeout=25):
            try:
                for t in fut.result(timeout=10):
                    key = (t["ticker"], t["insider"], t["trade_date"], round(t["value"], -2))
                    if key in merged:
                        merged[key]["sector"] = t["sector"]  # SIC autoritativo
                    else:
                        merged[key] = t
            except:
                pass
    ...
    if not edgar_ok:
        warning = "⚡ EDGAR non raggiungibile — solo OpenInsider (latenza 30-60 min)"

    return list(merged.values()), source_label, warning

# ─── YFINANCE ─────────────────────────────────────────────────────────────────
# yfinance rimosso — nessuna dipendenza esterna
...
# ─── FETCH SETTORE PER NON CLASSIFICATI ───────────────────────────────────────
@st.cache_data(ttl=86400, show_spinner=False)  # cache 24h
def fetch_sector_label(ticker: str) -> str:
    """
    Usa Yahoo Finance /v1/finance/search (endpoint pubblico, no auth)
    per ottenere il settore di un ticker non classificato localmente.
    Restituisce stringa settore o "—" se fallisce.
    """
    try:
        url = (f"https://query2.finance.yahoo.com/v1/finance/search"
               f"?q={ticker}&newsCount=0&enableFuzzyQuery=false&enableCb=false")
        ...
        for q in quotes:
            if q.get("symbol","").upper() == ticker.upper():
                sector = q.get("sector") or q.get("industry") or "—"
                return sector[:30] if sector else "—"
        return "—"
    except:
        return "—"
...
def render_card(t):
    ...
    sector = t.get("sector","—")
    if sector == "—":
        fetched = fetch_sector_label(t["ticker"])
        if fetched and fetched != "—":
            sector = fetched[:30]
    sc,sbg,sbd = sector_badge_style(sector if sector in SECTOR_COLORS else "Non classificati")
    sector_badge_html = (
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    fetch_result = fetch_trades(vl, vh)
    if len(fetch_result) == 3:
        trades, data_source, edgar_warn = fetch_result
        err = edgar_warn
    else:
        trades, err = fetch_result
    ...
    # Filtro settori
    if sel_sectors:
        if "Non classificati" in sel_sectors:
            other = [s for s in sel_sectors if s != "Non classificati"]
            trades = [t for t in trades
                      if t.get("sector","—") in other
                      or t.get("sector","—") == "—"]
        else:
