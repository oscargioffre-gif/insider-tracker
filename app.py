"""OS OpenInSider v14 — Scraping ultra-robusto, multi-strategy, cache-busting"""

import streamlit as st
import requests, re, json, time, random, io, csv
...
.strategy-badge{font-size:.65rem;font-weight:700;color:#6b7280;background:#0a0f14;border:1px solid #1a2535;border-radius:4px;padding:1px 6px;display:inline-block;margin-left:6px}
...
# Pool di User-Agent reali Chrome/Firefox/Edge — ruotati casualmente
_UA_POOL = [
...
# Struttura colonne di OpenInsider (tinytable) — GROUND TRUTH aggiornata
# idx: 0=X(link SEC), 1=FilingDate, 2=TradeDate, 3=Ticker, 4=Company,
#      5=Insider, 6=Title, 7=Type, 8=Price, 9=Qty, 10=Owned, 11=ΔOwn,
#      12=Value, 13=1d%, 14=1w%, 15=1m%, 16=6m%
_OI_COL_FALLBACK = {
    "filing_date": 1, "trade_date": 2, "ticker": 3, "company": 4,
    "insider": 5, "title": 6, "type": 7, "price": 8,
    "qty": 9, "value": 12,  # <-- VALUE è col 12, NON 13!
}
...
def classify_sector(company: str) -> str:
    ...
    for sector in _SECTOR_PRIORITY:
        if any(kw in cl for kw in SECTOR_KEYWORDS.get(sector, [])):
            return sector
    return "—"
...
SECTOR_SIC_RANGES = {
    "Biotech": [(2830,2836),(2860,2869),(3826,3827),(3841,3845),(3851,3851),
                (5047,5047),(5122,5122),(8000,8099),(8731,8731),(8734,8734)],
    "Semiconductors": [(3674,3674),(3670,3679),(3825,3826),(3559,3559)],
    "Technology": [(7372,7372),(7371,7371),(7373,7374),(7375,7379),
                   (3570,3579),(3669,3669),(4813,4813),(4899,4899)],
    "Industrials": [(3400,3499),(3500,3599),(3710,3716),(3720,3729),
                    (3730,3743),(3760,3769),(4400,4499),(4500,4599),(4210,4215)],
}
...
def _parse_date(s: str) -> str:
    """Prova più formati data e restituisce YYYY-MM-DD o stringa vuota."""
    if not s: return ""
    s = s.strip()
    for fmt in ("%Y-%m-%d", "%m/%d/%Y", "%m/%d/%y", "%Y/%m/%d",
        ...
        try:
            ...
            pass
    # Ultimo tentativo: prendi solo i primi 10 char se sembrano una data
...
def _make_headers(ua: str | None = None) -> dict:
    ...
    return {
        ...
        "Accept-Encoding": "gzip, deflate, br" if not is_ff else "gzip, deflate, br",
        "DNT": "1",
        "Connection": "keep-alive",
        "Upgrade-Insecure-Requests": "1",
        "Sec-Fetch-Dest": "document",
        "Sec-Fetch-Mode": "navigate",
        "Sec-Fetch-Site": "same-origin" if not is_ff else "same-origin",
        "Sec-Fetch-User": "?1",
...
# ─── URL BUILDER ──────────────────────────────────────────────────────────────
def build_url(vl: int, vh: int, days: int = 7, page: int = 1, cnt: int = 200) -> str:
    """Costruisce l'URL del screener OpenInsider.
    fd=days  → filing date negli ultimi N giorni (default 7, uguale all'URL originale)
    td=0     → nessun filtro su trade date
    cnt=200  → max risultati per pagina
    """
    ...
    return (
        ...
        f"&isdirector=1&istenpercent=1&isother=1"
        f"&grp=0&nfl=&nfh=&nil=&nih=&nol=&noh=&v2l=&v2h=&oc2l=&oc2h="
        f"&sortcol=0&cnt={cnt}&page={page}&Action=Submit"
    )


# ─── COLUMN DETECTION: 3 METODI A CASCATA ────────────────────────────────────
def _detect_columns_by_header(header_row) -> dict:
    """Metodo 1: analisi intestazioni della tabella."""
    cols = {}
    cells = header_row.find_all(["th", "td"])
    for i, cell in enumerate(cells):
        txt = cell.get_text(strip=True).lower()
        # Filing Date — deve venire PRIMA di trade date
        if "filing" in txt and "date" in txt and "filing_date" not in cols:
            cols["filing_date"] = i
        elif "trade" in txt and "date" in txt and "trade_date" not in cols:
            cols["trade_date"] = i
        elif txt in ("ticker", "sym", "symbol") and "ticker" not in cols:
            cols["ticker"] = i
            ...
            cols["title"] = i
        elif txt in ("trade type", "type", "transaction") and "type" not in cols:
            cols["type"] = i
        elif "price" in txt and "price" not in cols:
            cols["price"] = i
        elif txt in ("qty", "quantity", "shares") and "qty" not in cols:
            cols["qty"] = i
        # Value: cerca "value" ma NON "1d","1w","1m","6m" (% post-trade)
        elif "value" in txt and not any(x in txt for x in ["1d","1w","1m","6m","%"]) and "value" not in cols:
            cols["value"] = i
    return cols

def _detect_columns_by_content(data_rows, n_sample: int = 5) -> dict:
    """Metodo 2: inferisce colonne dal contenuto delle prime N righe.
    Cerca pattern tipici: date ISO, ticker uppercase, valori $ grandi.
    """
    if not data_rows: return {}
    # Campiona fino a n_sample righe
    sample = [r.find_all("td") for r in data_rows[:n_sample] if r.find_all("td")]
    ...
    value_re  = re.compile(r"^\$?[\d,]+$")  # numero grande senza decimali
    price_re  = re.compile(r"^\$?[\d]+\.\d{2}$")  # numero con 2 decimali

    col_scores: dict[str, dict[int,int]] = {k:{} for k in
        ["filing_date","trade_date","ticker","value","price"]}

    for row in sample:
        for i, td in enumerate(row):
            txt = td.get_text(strip=True)
            if date_re.match(txt):
                # Prima data = filing_date (col 1), seconda = trade_date (col 2)
                col_scores["filing_date"][i] = col_scores["filing_date"].get(i,0)+1
                col_scores["trade_date"][i]  = col_scores["trade_date"].get(i,0)+1
            if ticker_re.match(txt) and len(txt)<=5:
                col_scores["ticker"][i] = col_scores["ticker"].get(i,0)+1
            if value_re.match(txt) and len(re.sub(r"[^\d]","",txt))>=5:
                col_scores["value"][i] = col_scores["value"].get(i,0)+1
            if price_re.match(txt):
                col_scores["price"][i] = col_scores["price"].get(i,0)+1

    cols: dict[str,int] = {}
    # Filing date: colonna con il punteggio più alto tra quelle < trade_date
    # Assumi che filing_date < trade_date per indice (filing viene prima nel display)
    date_cols = sorted(col_scores["filing_date"].items(), key=lambda x: (-x[1],x[0]))
    if date_cols:
        best_date_cols = [c for c,s in date_cols if s >= 1][:2]
        if best_date_cols:
            cols["filing_date"] = best_date_cols[0]
            if len(best_date_cols) > 1:
                cols["trade_date"] = best_date_cols[1]
    ticker_best = sorted(col_scores["ticker"].items(), key=lambda x:-x[1])
    if ticker_best and ticker_best[0][1] >= 1:
        cols["ticker"] = ticker_best[0][0]
    # Value: cerca la colonna con valori > 4 cifre, più a destra del ticker
    ticker_idx = cols.get("ticker", 3)
    value_cands = [(i,s) for i,s in col_scores["value"].items() if i > ticker_idx]
    if value_cands:
        cols["value"] = max(value_cands, key=lambda x: x[1])[0]
    price_cands = [(i,s) for i,s in col_scores["price"].items() if i > ticker_idx]
    if price_cands:
        cols["price"] = max(price_cands, key=lambda x: x[1])[0]
    return cols

def _merge_cols(a: dict, b: dict, fallback: dict) -> dict:
    """Fonde 3 fonti di column map: header > content > fallback."""
    result = dict(fallback)
    result.update({k:v for k,v in b.items() if v is not None})
    result.update({k:v for k,v in a.items() if v is not None})
    return result


# ─── TABLE FINDER ─────────────────────────────────────────────────────────────
def _find_data_table(soup: BeautifulSoup):
    """Cerca la tabella dati principale con 3 strategie."""
    # 1. Per classe nota
    for cls in ["tinytable", "sortable", "table-striped", "insider-table"]:
        t = soup.find("table", class_=lambda c, _cls=cls: c and _cls in c)
        if t and len(t.find_all("tr")) > 2:
            return t
    # 2. Per tabella con più righe
    tables = soup.find_all("table")
    if not tables: return None
    best = max(tables, key=lambda t: len(t.find_all("tr")), default=None)
    return best if best and len(best.find_all("tr")) > 2 else None


# ─── ROW PARSER ───────────────────────────────────────────────────────────────
def _parse_table(soup: BeautifulSoup, vl_val: float) -> list[dict]:
    """Estrae trade dalla tabella HTML con rilevamento colonne a 3 livelli."""
    ...
    # Rileva colonne
    header_cols  = _detect_columns_by_header(all_rows[0])
    content_cols = _detect_columns_by_content(all_rows[1:])
    cols = _merge_cols(header_cols, content_cols, _OI_COL_FALLBACK)
    ...
    for tr in all_rows[1:]:
        ...
        try:
            ...
            # Filtra solo acquisti (P = Purchase)
            ttype = get("type","")
            if (ttype.split()[0].upper() if ttype else "") != "P":
                continue

            # Date — parse robusto multi-formato
            filing_raw  = get("filing_date")
            trade_raw   = get("trade_date")
            filing_dt_s = _parse_date(filing_raw)
            trade_dt_s  = _parse_date(trade_raw)
            # Se la data di filing è vuota o malformata, skip
            if not filing_dt_s:
                continue
            # Orario (parte dopo la data)
            def _extract_time(raw: str) -> str:
                if not raw: return ""
                # Formato tipico: "2024-04-11 16:02:12"
                m = re.search(r"(\d{2}:\d{2}(?::\d{2})?)", raw)
                return m.group(1) if m else ""
            filing_time = _extract_time(filing_raw)
            trade_time  = _extract_time(trade_raw)

            # Ticker — estrai dal link <a> se presente
            ticker_td = tds[cols.get("ticker", 3)]
            ticker_a  = ticker_td.find("a")
            ticker_raw = (ticker_a.get_text(strip=True) if ticker_a
                          else ticker_td.get_text(strip=True))
            ticker = re.sub(r"[^A-Z.]", "", ticker_raw.upper().strip())
            if not ticker or len(ticker) > 6: continue

            company = tds[cols.get("company", 4)].get_text(strip=True)

            # Insider — estrai dal link <a> se presente
            ...
            price = parse_num(get("price", "0"))
            qty   = parse_num(get("qty",   "0"))
            value = parse_num(get("value", "0"))

            # Se value è 0 o mancante, calcola da qty * price
...
def _human_delay(base: float = 0.8, jitter: float = 1.2) -> None:
    """Pausa random che simula latenza umana."""
    time.sleep(base + random.uniform(0, jitter))


def _strategy_cloudscraper(url: str) -> tuple[BeautifulSoup | None, str]:
    """Strategia 1: cloudscraper con fingerprint Chrome completo."""
    if not _HAS_CS:
        return None, "cloudscraper non disponibile"
    try:
        scraper = cloudscraper.create_scraper(
            browser={"browser": "chrome", "platform": "windows", "mobile": False},
            delay=2,
        )
        # Prima visita homepage (simula navigazione umana)
        scraper.get("http://openinsider.com/", timeout=15)
        _human_delay(0.5, 1.0)
        r = scraper.get(url, timeout=30)
        if r.status_code == 200 and len(r.text) > 5000:
            return BeautifulSoup(r.text, "lxml"), "cloudscraper"
        return None, f"cloudscraper HTTP {r.status_code}"
    except Exception as e:
        return None, f"cloudscraper error: {e}"


def _strategy_requests_session(url: str) -> tuple[BeautifulSoup | None, str]:
    """Strategia 2: requests.Session con cookie, referer chain e headers completi."""
    try:
        ua = random.choice(_UA_POOL)
        sess = requests.Session()
        sess.headers.update(_make_headers(ua))
        # Step 1: homepage per ottenere cookie di sessione
        sess.get("http://openinsider.com/", timeout=15)
        _human_delay(0.6, 1.4)
        # Step 2: pagina screener
        r = sess.get(url, timeout=30)
        if r.status_code == 200 and len(r.text) > 5000:
            return BeautifulSoup(r.text, "lxml"), "requests+session"
        return None, f"requests HTTP {r.status_code}"
    except Exception as e:
        return None, f"requests error: {e}"


def _strategy_requests_direct(url: str) -> tuple[BeautifulSoup | None, str]:
    """Strategia 3: requests diretta con UA diverso e retry esponenziale."""
    for attempt in range(1, 4):
        try:
            ua = _UA_POOL[(attempt - 1) % len(_UA_POOL)]
            headers = _make_headers(ua)
            headers["Cache-Control"] = "no-cache, no-store"
            headers["Pragma"] = "no-cache"
            r = requests.get(url, headers=headers, timeout=25)
            if r.status_code == 200 and len(r.text) > 5000:
                return BeautifulSoup(r.text, "lxml"), "requests-direct"
            if attempt < 3:
                _human_delay(attempt * 1.5, 1.0)
        except Exception as e:
            if attempt < 3:
                _human_delay(attempt * 2.0, 1.0)
    return None, "tutti i 3 tentativi diretti falliti"


_STRATEGIES = [
    ("cloudscraper",       _strategy_cloudscraper),
    ("requests+session",   _strategy_requests_session),
    ("requests-direct",    _strategy_requests_direct),
]


# ─── FETCH PRINCIPALE ─────────────────────────────────────────────────────────
@st.cache_data(ttl=60, show_spinner=False)
def fetch_trades(vl: int, vh: int, _cache_buster: str = "") -> tuple[list[dict], str, str]:
    """
    Fetch robusto con 3 strategie a cascata.
    Restituisce (trades, error_msg, strategy_used).
    Cache 60s — invalidata su ogni nuova sessione browser tramite cache_buster.
    fd=7 giorni (coerente con URL originale del problema).
    Viene fatto fetch di 2 pagine se la prima ha >= 100 risultati.
    """
    vl_val  = vl * 1000
    ...
    for page in range(1, 3):  # pagine 1 e 2
        url = build_url(vl, vh, days=7, page=page, cnt=200)
        page_soup = None

        for strat_name, strat_fn in _STRATEGIES:
            soup, err = strat_fn(url)
            if soup:
                page_soup = soup
        ...
        if page_soup is None:
            break  # impossibile raggiungere OI

        page_trades = _parse_table(page_soup, vl_val)
        all_trades.extend(page_trades)

        # Se la prima pagina ha meno di 100 risultati, non serve la pagina 2
        if page == 1 and len(page_trades) < 100:
            break

    # Deduplica per (ticker + filing_date + insider)
    seen: set[tuple] = set()
    deduped: list[dict] = []
    for t in all_trades:
        key = (t["ticker"], t["filing_date"], t["insider"])
        if key not in seen:
            seen.add(key)
            deduped.append(t)

    err_msg = "" if deduped else (last_err or "Nessun dato trovato")
    return deduped, err_msg, strategy_used


# ─── DISK CACHE ───────────────────────────────────────────────────────────────
def save_disk(trades: list[dict]) -> None:
    try:
...
def load_disk() -> tuple[list[dict], str]:
    try:
...
# ─── AUTO-REFRESH ─────────────────────────────────────────────────────────────
def inject_autorefresh(secs: int) -> None:
...
def inject_autorefresh_off() -> None:
    st.markdown("""
<div class="ar-bar">
  <div class="ar-left">
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def main() -> None:
    # ── CACHE BUSTING SU NUOVA SESSIONE BROWSER ──────────────────────────────
    # Ogni volta che un utente apre una nuova scheda/sessione, svuotiamo la
    # cache in-memory di Streamlit così il prossimo fetch è SEMPRE fresco.
    if "session_init" not in st.session_state:
        st.cache_data.clear()
        st.session_state["session_init"] = True
        # cache_buster = timestamp della sessione (forza nuova chiave cache)
        st.session_state["cache_buster"] = datetime.now(ROME).strftime("%Y%m%d%H%M")
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    st.markdown('<div class="fc-label">&#x1F4B0; Valore minimo acquisto ($K)</div>',
                unsafe_allow_html=True)
    vl = st.slider("vl", 15, 10_000, 15, 25,
                   label_visibility="collapsed", format="$%dK")
    st.markdown(
    ...
    if last_vl is not None and last_vl != vl:
        ...
        # Nuovo valore minimo → forza nuovo cache buster
        st.session_state["cache_buster"] = datetime.now(ROME).strftime("%Y%m%d%H%M%S")
    ...
    if auto_on:
        freq = st.select_slider("Frequenza", options=list(REFRESH_OPTIONS.keys()),
                                value="5 min")
    ...
    trades, err, strat = fetch_trades(vl, vh, _cache_buster=cache_buster)
    ...
    if using_stale:
        st.markdown(
            f'<div class="stale-banner">&#x1F4E6; Dati salvati ({stale_ts[:16]} IT)</div>',
            unsafe_allow_html=True)
    ...
    # Statistiche globali (tutti i 7 giorni, tutti i settori)
    ...
    st.markdown(
        ...
        f'(soglia ${vl}K)'
        f'<span class="strategy-badge">{strat}</span>'
        f'</div>',
        unsafe_allow_html=True)
    ...
    # Ordina per filing_date DESC, poi filing_time DESC
    trades = sorted(trades,
                    key=lambda x: (x.get("filing_date",""), x.get("filing_time","")),
                    reverse=True)
...
<div class="stats-bar">
  ...
  <div class="stat-ts">{now_s} IT &middot; OpenInsider &middot; Form 4 &middot; {strat}</div>
</div>""", unsafe_allow_html=True)
