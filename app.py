...
def is_clevel(title):
    if not isinstance(title, str) or not title.strip():
        return False
    tl = title.lower()
    # Includi esplicitamente i 10% owner (grandi acquirenti strutturali)
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level Purchases &middot; SEC Form 4</div>
    ...
    # FIX 3: se vl/vh cambiati rispetto all'ultima run, invalida i risultati
    ...
    # Filtro settori
    if sel_sectors:
        # FIX 1: match flessibile — "Financial Services" batte su "Financials" e viceversa
        def sector_match(trade_sector, chosen):
            ts = trade_sector.lower()
            return any(c.lower() in ts or ts in c.lower() for c in chosen)
