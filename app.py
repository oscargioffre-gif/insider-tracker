...
SECTOR_KEYWORDS = {
    ...
    "Technology": [
        ...
        "electric vehicle","ev charging","chargepoint","charge point",
        "e-commerce","ecommerce","commerce","marketplace","baozun",
        "exzeo","charging station","smart grid","cleantech","clean tech",
        ...
        "quantum","quanta","itron","trimble","calix","viavi",
        "ciena","infinera","adtran","ribbon","sonus",
    ],
    "Industrials": [
...
def _build_sic_url(vl: int, sicl: int, sich: int) -> str:
    """URL OpenInsider con filtro SIC — fonte autorevole settore."""
    return (
        f"http://openinsider.com/screener?s=&o=&pl=&ph=&ll=&lh="
        f"&fd=30&fdr=&td=0&tdr=&fdlyl=&fdlyh=&daysago="
        f"&xp=1&xs=0&xa=0&xd=0&xg=0&xf=0&xm=0&xx=0&xo=0"
        f"&vl={vl}&vh="
...
def fetch_trades(vl, vh):
    """
    Fetch principale unico da OpenInsider — 1 chiamata HTTP.
    Cache 2 min.
    """
...
@st.cache_data(ttl=120, show_spinner=False)
def fetch_sector_sic(vl: int, sector: str) -> list:
    """
    Fetch SIC settoriale — 1 chiamata sequenziale per settore.
    Usata solo quando l'utente filtra per un settore specifico.
    Garantisce che tutte le aziende con quel SIC siano incluse,
    indipendentemente dal nome (es. ChargePoint, Baozun, Exzeo).
    """
    ...
    for sicl, sich in ranges:
        try:
            r = oi_safe_fetch(_build_sic_url(vl, sicl, sich), retries=2, delay=3)
            if r is None:
                continue
            rows = _parse_table(BeautifulSoup(r.text, "lxml"), cutoff)
            for t in rows:
                if t["value"] >= vl_val:
                    t["sector"] = sector
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    # Filtro settori: keyword classification + SIC fetch sequenziale per completezza
    if sel_sectors:
        # Step 1: arricchisci con SIC fetch per ogni settore selezionato
        # (cattura aziende come ChargePoint, Baozun, Exzeo che non matchano keyword)
        sic_extra = {}
        for sector in sel_sectors:
            for t in fetch_sector_sic(vl, sector):
                key = (t["ticker"], t["insider"], t["trade_date"], round(t["value"], -2))
                sic_extra[key] = t
        # Merge: aggiungi trade SIC non già presenti, aggiorna settore di quelli presenti
        existing_keys = {
            (t["ticker"], t["insider"], t["trade_date"], round(t["value"], -2))
            for t in trades
        }
        for key, t in sic_extra.items():
            if key not in existing_keys:
                trades.append(t)
            else:
                # aggiorna settore con SIC (autoritativo)
                for tr in trades:
                    if (tr["ticker"], tr["insider"], tr["trade_date"], round(tr["value"],-2)) == key:
                        tr["sector"] = t["sector"]
                        break
        # Step 2: filtra per settori selezionati
