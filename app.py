...
def is_clevel(title):
    ...
    tl = title.lower()
    if "10%" in tl and "owner" in tl: return True
...
def build_url(vl, vh, days=30):
    ...
    return (
        ...
        f"&isdirector=0&is10percent=1&isother=0"
...
def _parse_table(soup, cutoff):
    ...
    for tr in all_rows[1:]:
        ...
        try:
            ...
            title=get("title")
            if "10%" in title.lower() and "owner" in title.lower():
                role_abbr,role_full="10%","10% Owner"
            elif not is_clevel(title): continue
            else: role_abbr,role_full=extract_role(title)
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level + 10% Owner &middot; SEC Form 4</div>
    ...
    st.markdown('<div class="pill-label">&#x1F3ED; Settori &mdash; tocca per filtrare</div>',
                unsafe_allow_html=True)
    sel_sec_labels = st.pills("Settori", ALL_SECTORS, selection_mode="multi",
...
<div class="stats-bar">
  ...
  <div class="stat-ts">{now_s} IT &middot; OpenInsider &middot; C-Level + 10% Owner &middot; Form 4</div>
</div>""", unsafe_allow_html=True)
