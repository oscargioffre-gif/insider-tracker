"""
OS OpenInSider  v9
• http://openinsider.com (HTTP porta 80, Chrome UA, BeautifulSoup, retry x3)
• 7 quadratini-giorno con glow al touch (st.pills)
• Settori come pill luminose (st.pills)
• Auto-refresh JS countdown
"""
...
button[data-testid="stPillsButton"]:hover{
  ...
  box-shadow:
    ...
    0 0 6px rgba(16,185,129,.2)!important;
  transform:translateY(-2px) scale(1.05)!important;
}

/* selezionato — acceso */
button[data-testid="stPillsButton"][aria-selected="true"]{
  ...
  box-shadow:
    ...
    0 0 8px rgba(16,185,129,.3),
    inset 0 0 20px rgba(16,185,129,.1)!important;
  text-shadow:0 0 14px rgba(16,185,129,.8)!important;
  transform:translateY(-1px)!important;
}

/* selezionato + hover — massimo brillio */
button[data-testid="stPillsButton"][aria-selected="true"]:hover{
  color:#d1fae5!important;
  box-shadow:
    0 0 0 2px rgba(16,185,129,.7),
    0 0 32px rgba(16,185,129,.7),
    0 0 14px rgba(16,185,129,.4),
    inset 0 0 24px rgba(16,185,129,.15)!important;
  text-shadow:0 0 18px rgba(16,185,129,1)!important;
...
<div class="app-header">
  ...
  <div class="header-sub"><div class="live-dot"></div>C-Level Purchases &middot; SEC Form 4</div>
    ...
    # ── 7 quadratini giorno (st.pills) ───────────────────────
    today = datetime.utcnow().date()
    day_options = []   # label visibile
    label_to_date = {} # label → "YYYY-MM-DD"
    for i in range(7):
    ...
    st.markdown('<div class="pill-label">📅 Giorni — tocca per filtrare</div>', unsafe_allow_html=True)
    sel_day_labels = st.pills(
        "Giorni", day_options,
        selection_mode="multi",
        default=day_options[:2],   # Oggi + Ieri accesi di default
        key="day_pills",
        label_visibility="collapsed",
    )
    selected_dates = {label_to_date[l] for l in (sel_day_labels or [])}

    # ── Aggiorna + Auto-refresh ───────────────────────────────
    ...
    sel_sectors = []
    if all_sectors:
        st.markdown('<div class="pill-label">🏭 Settori — tocca per filtrare</div>', unsafe_allow_html=True)
    ...
    if sel_sectors:
        trades = [t for t in trades if enriched_map[t["ticker"]].get("sector","—") in sel_sectors]
    if not trades:
...
<div class="stats-bar">
  ...
  <div class="stat-ts">{now_s} UTC · openinsider.com · C-Level · Form 4</div>
</div>""", unsafe_allow_html=True)
