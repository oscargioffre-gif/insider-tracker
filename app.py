...
def fetch_trades(vl, vh):
    """
    Tenta SEC EDGAR (latenza 5-15 min).
    Se EDGAR fallisce, usa OpenInsider come fallback (latenza 30-60 min).
    Returns: (trades, source_label, error_message)
    source_label: "EDGAR" | "OpenInsider"
    """
    today    = datetime.utcnow().date()
    start_dt = (today - timedelta(days=30)).strftime("%Y-%m-%d")
    end_dt   = today.strftime("%Y-%m-%d")

    # ── Step 1: paginate EFTS ──────────────────────────────────────────────────
    hits_all = []
    from_idx = 0
    ...
    if not hits_all:
        # Fallback a OpenInsider
        oi_trades, oi_err = _fetch_from_oi(vl, vh)
        if oi_trades:
            return oi_trades, "OpenInsider", "EDGAR non disponibile — usando OpenInsider"
        return [], "OpenInsider", "Nessuna fonte raggiungibile"

    # ── Step 2: fetch + parse XMLs in parallel ─────────────────────────────────
    def process_hit(hit):
        try:
            src = hit.get("_source", {})
            acc = hit.get("_id","")
    ...
    # ── Step 4: deduplicate (same insider, same ticker, same trade date, ~same value)
    seen, deduped = set(), []
    for t in all_trades:
    ...
    if not deduped:
        # EDGAR ha risposto ma zero risultati — prova OI come verifica
        oi_trades, oi_err = _fetch_from_oi(vl, vh)
        if oi_trades:
            return oi_trades, "OpenInsider", "EDGAR: 0 risultati — usando OpenInsider"
        return [], "EDGAR", ""
    return deduped, "EDGAR", ""
